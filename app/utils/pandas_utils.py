...
def do_basic_statistics_by_column(df: Any, column: str) -> dict:
    df_filtered: pd.DataFrame = remove_not_numbers_values(remove_percentage(df[column]))
    return {
...
def remove_percentage(df: Any) -> pd.DataFrame:
    return df.apply(lambda x: float(x.replace('%', '')) if isinstance(x, str) and '%' in x else x)
                    
def remove_not_numbers_values(df: Any) -> pd.DataFrame:
